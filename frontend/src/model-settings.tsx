...
export function ModelSettings({api,onSaved}:{api:API;onSaved:()=>void}){
  ...
  return <section className="modelHub">
    ...
    {draft&&<div className="panel settingsForm modelEditor"><div className="modelHeader"><h2>{draft.id?'编辑模型':'添加模型'}</h2><button disabled={busy} onClick={()=>{setDraft(null);setDirty(false);setKey('');}}>关闭编辑</button></div><fieldset disabled={busy}><legend>连接设置</legend>
