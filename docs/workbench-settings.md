...
## 保存前获取模型列表（2026-09-14）

新增模型或编辑模型时，填写 API 根地址及密钥后即可点击“从服务器获取模型列表”，
不再要求先保存或提前知道模型 ID。服务器结果以明确的下拉框展示，选择后再保存。
POST /api/settings/models/discover 使用当前表单，不写入模型库；留空密钥只在原条目的
端点与协议相同时复用原密钥，切换端点不会转发旧密钥。原按 ID 查询接口保留。
列表可见并不保证 Chat Completions 或任务动作格式可用，保存后仍应测试。
错误显示 HTTP 状态及固定诊断提示，不回显服务器原始错误正文或凭据。
本机现存 Kimi 配置的 /models 查询返回 HTTP 401，需检查当前平台密钥，不能通过模型选择绕过鉴权。


### 密钥显示与列表就地反馈

编辑模型支持“显示 API Key / 隐藏 API Key”。新输入密钥可切换明文；已保存密钥仅在
用户点击后通过同源 POST /api/settings/models/{id}/reveal-key 返回，响应不缓存。
常规模型列表和详情仍不返回密钥。隐藏、关闭编辑或更换端点清除前端已揭示的密钥。
显示已保存密钥不会自动填入新密钥输入框，避免编辑地址时将旧密钥带到新服务。
获取模型的加载状态、错误或空列表信息显示在按钮下面；成功模型列表直接展示为可选按钮，
同时保留模型下拉框。当前 Kimi 鉴权 401 时无法取得真实列表，不展示虚构模型。
