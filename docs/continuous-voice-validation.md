...
新增后端转写预览、待处理输入、LLM 草稿与 SSE 推送。76 项 Python、10 项 Node
测试通过，覆盖预览不提交字段、LLM 未返回前显示输入、NDJSON 分块回调与中途断流。
实际 Docker 合成语音测试收到 6 个不同的草稿文本，第二轮 0.500 秒收到“费用”，
0.797 秒收到完整回复草稿，2.406 秒收到最终校验结果；转写在第一个 SSE 快照已出现。
见 [流式原始记录](streaming-smoke.json)。以上以 SSE 连接建立为计时起点，不是
真实麦克风端到端延迟；未验证真实浏览器设备。TTS 播放仍等待最终校验。
